import { initializeApp } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-app.js";
        ...
        function updateLanguage() {
            ...
            // Keep the Game Info modal in sync if it happens to be open while switching language.
            try {
                const infoModal = document.getElementById('info-modal');
                if (infoModal && !infoModal.classList.contains('hidden')) renderInfoModalBody();
            } catch (e) { /* modal not ready yet — ignore */ }
        }
        ...
        // Renders the Game Info modal body.
        // Default = the built-in Arabic content written directly in index.html
        //           (#info-modal-default). It is shown in BOTH language modes on purpose.
        // Override = if an admin saves text in the CMS (Game Info Management), that text
        //            replaces the default content entirely.
        function renderInfoModalBody() {
            const infoTextEl = document.getElementById('info-modal-text');
            const defaultEl = document.getElementById('info-modal-default');
            if (!infoTextEl || !defaultEl) return;

            const customText = gameInfoData
                ? ((currentLanguage === 'ar' ? gameInfoData.infoText_ar : gameInfoData.infoText_en)
                    || gameInfoData.infoText_ar || gameInfoData.infoText_en || '').trim()
                : '';
        ...
        // Kept as an alias so any older call site still works — single source of truth above.
        ...
        function handleLogin() {
            ...
            signInWithEmailAndPassword(auth, email, password)
                .then(async (userCredential) => {
                    ...
                    if (!currentUserData) {
                        if (email.includes('teacher')) currentUserData = { id: currentUser.uid, username: 'Teacher', email, role: 'teacher' };
                        else if (email.includes('admin')) currentUserData = { id: currentUser.uid, username: 'Admin', email, role: 'admin' };
                        else currentUserData = { id: currentUser.uid, username: email.split('@')[0], email, role: 'student', grade: 'first' };
                    }
                    navigateToUserPage();
                })
                .catch(error => {
